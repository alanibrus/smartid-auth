...
class AuthHash {
  generateRandomHash(hashType = 'sha512') {
    return new Promise((resolve, reject) =>
      this.getRandomBytes().then(buf =>
        resolve({ raw: buf, digest: crypto.createHash(hashType).update(buf).digest('base64') })
  ...
  calculateVerificationCode(rawHashInBase64) {
    ...
    const sha256HashedInput = crypto.createHash('sha256').update(Buffer.from(rawHashInBase64, 'base64')).digest();
